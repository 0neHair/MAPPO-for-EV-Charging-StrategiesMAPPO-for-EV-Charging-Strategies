...
class EV_Agent():
    ...
    def step(self, time, train=True):
        ...
        if self.stop_update == False and self.is_active == True:
            if self.is_charging: 
                ...
                if self.waiting_charging_time <= 0:
                ...
                self.total_run_dis += self.frame * self.speed / 100
    ...
    def set_caction(self, caction=0, waiting_time=0, charging_time=0):
        '''
        设置充电动作，并执行
    ...
    def set_raction(self, raction=0, reset_record=False):
        ...
        self.time_to_next = dis_to_next / self.speed * 100 # 路段行驶时间
        self.total_used_time += self.time_to_next
        reward -= self.time_to_next/100*self.travel_time_beta
    ...
    def get_choice_set(self):
        return self.map_adj_index[self.target_pos]
    ...
    def get_choice_set_mask(self):
        ...
        return self.map_adj[self.target_pos]
