...
class Multi_EV_Env(gym.Env):
        ...
        for i, agent in enumerate(self.agents):
            ...
            if agent.is_choosing:
                ...
                if agent.target_pos in self.near_end:
                    ...
                    consume_SOC = dis_to_next  * agent.consume / agent.E_max
                    if agent.SOC - consume_SOC < agent.SOC_exp:
                        tSOC = consume_SOC + agent.SOC_exp + 0.01
                        rtSOC = round(tSOC, 1)
                        if rtSOC < tSOC:
                            rtSOC += 0.05
                        caction_n = self.caction_list.index(rtSOC)
                else:
                    if agent.SOC < 0.15:
                        caction_n = len(self.caction_list)-1

                self.set_n_caction(agent, caction_n) # 设置动作
                if caction_n == 0:
    ...
    def get_obs(self, agent: EV_Agent):
        ...
        return np.array(cobs), choice_set_mask
