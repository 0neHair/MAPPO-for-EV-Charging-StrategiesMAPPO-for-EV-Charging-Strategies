...
class QNetwork(nn.Module):
    def __init__(self, share_dim, action_dim, value_arch):
        ...
        value_net.append(layer_init(nn.Linear(last_layer_dim, action_dim), gain=1.0))
...
class cNetwork(nn.Module):
    def __init__(
        ...
        self.qf1 = QNetwork(share_dim, action_dim, value_arch)
        self.qf2 = QNetwork(share_dim, action_dim, value_arch)
        self.qf1_target = QNetwork(share_dim, action_dim, value_arch)
        self.qf2_target = QNetwork(share_dim, action_dim, value_arch)
    ...
    def get_logits(self, state):
        ...
        log_prob = self.actor(state)
        masked_logit = log_prob.masked_fill((~mask).bool(), -1e32)
        return Categorical(logits=masked_logit), F.log_softmax(masked_logit, dim=1)
...
class rNetwork(nn.Module):
    ...
    def get_logits(self, state, mask):
        log_prob = self.actor(state)
        masked_logit = log_prob.masked_fill((~mask.bool()), -1e32)
        return Categorical(logits=masked_logit), F.log_softmax(masked_logit, dim=1)