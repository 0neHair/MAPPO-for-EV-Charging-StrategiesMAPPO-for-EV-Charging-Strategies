    ...
    def __init__(
        ...
        if self.mode in ['NGH', 'OC']:
            ...
            self.cq_optimizer = torch.optim.Adam(
                list(self.charge_network.qf1.parameters())+list(self.charge_network.qf2.parameters()),
                lr=self.lr, eps=1e-5
            )
        ...
        if self.mode in ['NGH', 'OR']:
            ...
            self.rq_optimizer = torch.optim.Adam(
                list(self.route_network.qf1.parameters())+list(self.route_network.qf2.parameters()),
                lr=self.lr, eps=1e-5
            )
